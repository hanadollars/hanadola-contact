// api/submit-contact.js — Hanadola Contact Lead Capture
...
module.exports = async function handler(req, res) {
  ...
  if (!name || !phone) return res.status(400).json({ error: 'Thiếu thông tin name/phone' });
  if (phone.replace(/\D/g, '').length < 9) return res.status(400).json({ error: 'Số điện thoại không hợp lệ' });
  if (!services || services.length === 0) return res.status(400).json({ error: 'Vui lòng chọn ít nhất 1 dịch vụ' });
  ...
  console.log('[Lead] Saved:', leadId, '|', name, '|', phone);
  ...
  if (notifyEmail) {
    try {
      await sendEmail({
        to: notifyEmail,
        subject: `[Hanadola] Khách hàng mới – ${name} – ${phone}`,
...
<p style="font-size:12px;color:#4A6880;margin-bottom:20px">Hanadola Media & Technology · Contact Form</p>
<table style="width:100%;border-collapse:collapse;font-size:14px">
  ...
  <tr><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06);color:#8FA8C0">Email</td><td style="padding:10px 0;border-bottom:1px solid rgba(255,255,255,0.06)">${email || '—'}</td></tr>
...
<h1>Xin chào ${name}!</h1>
<p>Chúng tôi đã nhận được yêu cầu tư vấn của bạn. Đội ngũ Hanadola sẽ chủ động <strong style="color:#E8EDF2">nhắn Zalo</strong> cho bạn trong vòng <strong style="color:#3A7BD5">2 giờ làm việc</strong>.</p>
<div class="box">
  <p><strong>Dịch vụ quan tâm:</strong></p>
  <p style="color:#5B9BD5">${services.join(' · ')}</p>
  ${note ? `<p style="margin-top:8px"><strong>Ghi chú của bạn:</strong></p><p>${note}</p>` : ''}
</div>
<p>Nếu cần hỗ trợ ngay, hãy nhắn Zalo trực tiếp:</p>
